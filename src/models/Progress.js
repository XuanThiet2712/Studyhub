export class DayProgress {
  ...
  get checkCount() {
    return [this.grammarDone,this.vocabDone,this.listeningDone,this.readingDone,this.speakingDone].filter(Boolean).length;
  }
  get progressPct() { return Math.round(this.checkCount / 5 * 100); }

  markSection(section) {
    const map = { grammar:'grammarDone', vocab:'vocabDone', listening:'listeningDone', reading:'readingDone', speaking:'speakingDone' };
    if (map[section]) this[map[section]] = true;
    if (this.checkCount === 5 && !this.completed) {
      this.completed   = true;
  ...
  toJSON() {
    return {
      ...
      speaking_done: this.speakingDone, time_spent_mins: this.timeSpent,
      notes: this.notes
    };
  }
}
